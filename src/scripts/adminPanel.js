...
async function loadElections() {
    try {
        ...
        if (elections.length > 0) {
            ...
            // Weka buttons za actions
            const edActions = document.querySelector('.ed-actions');
            if (edActions) {
                edActions.innerHTML = `
                    <button class="btn btn-p btn-sm" onclick="openModal('createElectionModal')">+ New Election</button>
                    <button class="btn btn-gh btn-sm" onclick="pauseVoting(${el.id})">Pause Voting</button>
                    <button class="btn btn-d btn-sm" onclick="closeVotingEarly(${el.id})">Close Voting</button>
                `;
            }
        } else {
            const edActions = document.querySelector('.ed-actions');
            if (edActions) {
                edActions.innerHTML = `<button class="btn btn-p btn-sm" onclick="openModal('createElectionModal')">+ New Election</button>`;
            }
        }
...
async function createElection() {
    ...
    if (!title || !voting_opens || !voting_closes) { showToast('Jaza sehemu zote!', 'warning'); return; }
    try {
        ...
        const data = await res.json();
        if (res.ok) { showToast('Election imeundwa!', 'success'); closeModal('createElectionModal'); loadElections(); }
        else showToast(data.message || 'Kuna tatizo!', 'error');
    } catch(e) { showToast('Tatizo la mtandao!', 'error'); }
    finally { const btn = document.getElementById('create-election-btn'); if (btn) { btn.textContent = 'Create Election'; btn.disabled = false; } }
}
...
async function pauseVoting(electionId) {
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok) { showToast('Upigaji kura umesimamishwa!', 'warning'); loadElections(); }
        else showToast(data.message || 'Kuna tatizo!', 'error');
    } catch(e) { showToast('Tatizo la mtandao!', 'error'); }
}

async function closeVotingEarly(electionId) {
    if (!confirm('Funga upigaji kura sasa? Hatua hii haiwezi kubatilishwa.')) return;
    try {
        const res = await fetch(`${API_URL}/admin/elections/${electionId}/close`, {
            method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
        });
        const data = await res.json();
        if (res.ok) { showToast('Upigaji kura umefungwa!', 'success'); loadElections(); }
        else showToast(data.message || 'Kuna tatizo!', 'error');
    } catch(e) { showToast('Tatizo la mtandao!', 'error'); }
}
    ...
    try {
        ...
        data.positions.forEach(position => {
            position.candidates.forEach(candidate => {
                rows.push({ name: candidate.full_name, position: position.title, votes: candidate.votes_count || 0 });
            });
        });
...
async function addCandidate() {
    ...
    if (!full_name || !position_id) { showToast('Jaza jina na position!', 'warning'); return; }
    try {
        ...
        const data = await res.json();
        if (res.ok) {
            showToast('Mgombea ameongezwa!', 'success');
    ...
    finally { const btn = document.getElementById('cand-submit-btn'); if (btn) { btn.textContent = 'Add Candidate'; btn.disabled = false; } }
}
...
async function deleteCandidate(id) {
    ...
    try {
        ...
        if (res.ok) { showToast('Mgombea amefutwa!', 'success'); loadCandidates(); }
    } catch(e) { showToast('Tatizo la mtandao!', 'error'); }
}
...
function renderPositions(positions) {
    ...
    if (!positions || positions.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:30px;color:var(--muted)">Hakuna positions</td></tr>`;
        return;
    }
...
async function addPosition() {
    ...
    if (!title) { showToast('Jaza jina la position!', 'warning'); return; }
    try {
        ...
        const res = await fetch(`${API_URL}/admin/positions`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json', 'Accept': 'application/json' },
            body: JSON.stringify({ title, type, faculty: type==='f' ? faculty : null, election_id: 1 })
        });
        const data = await res.json();
        if (res.ok) { showToast('Position imeongezwa!', 'success'); closeModal('addPositionModal'); document.getElementById('pos-title').value=''; loadPositions(); }
        else showToast(data.message || 'Kuna tatizo!', 'error');
    } catch(e) { showToast('Tatizo la mtandao!', 'error'); }
    finally { const btn = document.getElementById('pos-submit-btn'); if (btn) { btn.textContent = 'Add Position'; btn.disabled = false; } }
}
...
function editPosition(id, title, type, faculty) {
    ...
    if (document.getElementById('editPosType')) { document.getElementById('editPosType').value = type; toggleEditFacSel(); }
    if (document.getElementById('edit-pos-faculty')) document.getElementById('edit-pos-faculty').value = faculty;
    openModal('editPositionModal');
}

function toggleEditFacSel() {
    const facGroup = document.getElementById('editFacSelGroup');
    if (facGroup) facGroup.style.display = document.getElementById('editPosType')?.value === 'f' ? '' : 'none';
}
...
async function updatePosition() {
    ...
    try {
        ...
        const data = await res.json();
        if (res.ok) { showToast('Position imesasishwa!', 'success'); closeModal('editPositionModal'); loadPositions(); }
        else showToast(data.message || 'Kuna tatizo!', 'error');
    } catch(e) { showToast('Tatizo la mtandao!', 'error'); }
    finally { const btn = document.getElementById('edit-pos-submit-btn'); if (btn) { btn.textContent = 'Save Changes'; btn.disabled = false; } }
}
...
async function deletePosition(id) {
    ...
    try {
        ...
        else { const data = await res.json(); showToast(data.message || 'Kuna tatizo!', 'error'); }
    } catch(e) { showToast('Tatizo la mtandao!', 'error'); }
}
...
async function loadPositionsDropdown() {
    try {
        ...
        if (!select) return;
        select.innerHTML = '<option value="">Select position...</option>' + positions.map(p => `<option value="${p.id}">${p.title}</option>`).join('');
    ...
    try {
        ...
        grid.innerHTML = elections.map(el => `
            <div class="past-card">
                ...
                <div class="past-body">
                    <div class="past-row"><span>Ilianza</span><span>${el.voting_opens ? new Date(el.voting_opens).toLocaleDateString() : '-'}</span></div>
                    <div class="past-row"><span>Ilimalizika</span><span>${el.voting_closes ? new Date(el.voting_closes).toLocaleDateString() : '-'}</span></div>
                    <div class="past-row"><span>Jumla ya Kura</span><span>${el.total_votes ?? '-'}</span></div>
                    <div class="past-row"><span>Ushiriki</span><span>${el.voter_turnout ? el.voter_turnout+'%' : '-'}</span></div>
                </div>
                <div class="past-foot">
...
async function exportPastElection(electionId) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            a.href = url; a.download = `election_${electionId}_results.xlsx`; a.click();
            URL.revokeObjectURL(url);
            showToast('Export imefanikiwa!', 'success');
        } else showToast('Tatizo la export!', 'error');
    } catch(e) { showToast('Tatizo la mtandao!', 'error'); }
}
    ...
    try {
        ...
        list.innerHTML = announcements.map(ann => `
            <div class="ann-item">
                ...
                <div class="ann-meta">Kwa: <strong>${ann.audience || 'Wote'}</strong>${ann.sent_count ? ` · Watu ${ann.sent_count} walipokea` : ''}</div>
            </div>
        `).join('');
...
async function sendAnnouncement() {
    ...
    if (!subject || !message) { showToast('Jaza kichwa na ujumbe!', 'warning'); return; }
    try {
    ...
    finally { const btn = document.getElementById('ann-send-btn'); if (btn) { btn.textContent = 'Send Announcement'; btn.disabled = false; } }
}
...
function renderNotifications(notifications) {
    ...
    const dotColor = (type) => type==='success'||type==='vote' ? 'g' : type==='warning' ? 'w' : type==='error' ? 'r' : 'g';
...
function filterNotifications(type) {
    ...
    if (type === 'unread') filtered = allNotifications.filter(n => !n.read_at);
    else if (type === 'system') filtered = allNotifications.filter(n => n.type === 'system');
...
async function downloadReport(type, format = 'pdf') {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = `kiutso_${type}_report.${format==='excel'?'xlsx':'pdf'}`; a.click();
            URL.revokeObjectURL(url);
            showToast('Report imepakiwa!', 'success');
        } else { const data = await res.json(); showToast(data.message || 'Tatizo kupakua report!', 'error'); }
    } catch(e) { showToast('Tatizo la mtandao!', 'error'); }
}
...
function renderVoters(voters) {
    ...
    tbody.innerHTML = voters.map(v => `
        <tr>
            ...
            <td>${v.votes && v.votes.length > 0 ? '<span class="badge badge-g"><span class="bdot"></span>Voted</span>' : '<span class="badge badge-w"><span class="bdot"></span>Not Voted</span>'}</td>
...
function filterV(t, el) {
    ...
    if (facultySelect && facultySelect.value !== 'All Faculties') filtered = filtered.filter(v => v.faculty === facultySelect.value);
...
function openVoterDetail(v) {
    ...
    if (rows[6]) rows[6].innerHTML = hasVoted ? '<span class="badge badge-g"><span class="bdot"></span>Voted</span>' : '<span class="badge badge-w"><span class="bdot"></span>Not Voted</span>';
    ...
    if (receiptSec) {
        if (hasVoted) {
            receiptSec.innerHTML = `<div class="vp-sec-title">Vote Receipt</div><p style="font-size:11.5px;color:var(--muted);margin-bottom:10px">Candidate choice is never disclosed.</p>${v.votes.map(vote => `<div class="vp-row"><span class="vp-k">${vote.position ? vote.position.title : 'Position '+vote.position_id}</span><span class="vp-v"><span class="badge badge-g">Voted</span></span></div>`).join('')}`;
        } else {
            receiptSec.innerHTML = `<div class="vp-sec-title">Vote Receipt</div><p style="font-size:11.5px;color:var(--muted)">This voter has not voted yet.</p>`;
        }
    }
...
function showToast(msg, type = 'success') {
    ...
    if (!document.getElementById('toast-style')) {
        ...
        style.textContent = '@keyframes toastin{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}@keyframes toastout{from{opacity:1}to{opacity:0;transform:translateY(10px)}}';
        document.head.appendChild(style);
    }
    const colors = { success:'#0a3018', error:'#dc2626', warning:'#f59e0b', info:'#1d4ed8' };
    const toast = document.createElement('div');
    toast.style.cssText = `background:${colors[type]||colors.success};color:#fff;padding:12px 18px;border-radius:10px;font-size:13.5px;font-family:'Poppins',sans-serif;font-weight:500;box-shadow:0 4px 20px rgba(0,0,0,.18);animation:toastin .25s ease;max-width:300px;line-height:1.5;`;
    toast.textContent = msg;
    container.appendChild(toast);
    setTimeout(() => { toast.style.animation='toastout .25s ease forwards'; setTimeout(()=>toast.remove(),250); }, 3500);
}

// ─── NAV ─────────────────────────────────────────────────────
const TITLES = {
    overview:'Overview', elections:'Manage Elections', positions:'Manage Positions',
    candidates:'Manage Candidates', results:'Vote Results', voters:'Registered Voters',
    announcements:'Announcements', reports:'Reports', notifications:'Notifications',
    logs:'System Logs', past:'Past Elections', settings:'Settings'
};
...
function closeAllDrops() { document.querySelectorAll('.notif-dropdown,.profile-dropdown').forEach(d => d.classList.remove('open')); }
function openModal(id) { document.getElementById(id).classList.add('open'); if (id==='addCandidateModal') loadPositionsDropdown(); }
...
function setBn(el) { document.querySelectorAll('.bn-item').forEach(b => b.classList.remove('on')); el.classList.add('on'); }
function selAud(el) { document.querySelectorAll('.aud-pill').forEach(p => p.classList.remove('on')); el.classList.add('on'); }
function switchSettings(el) {
    ...
    document.getElementById('sp-' + el.dataset.sp).classList.add('on');
    el.classList.add('on');
}
function toggleFacSel() {
    document.getElementById('facSelGroup').style.display = document.getElementById('posType').value === 'f' ? '' : 'none';
}

document.addEventListener('click', e => {
    if (!e.target.closest('[onclick*="toggleDrop"]') && !e.target.closest('.notif-dropdown') && !e.target.closest('.profile-dropdown')) closeAllDrops();
});
...
function handlePhotoUpload(e) {
    ...
    reader.onload = ev => {
        ...
        const img = document.getElementById('profilePhotoImg'); img.src=src; img.style.display='block'; document.getElementById('photoPreview').childNodes[0].textContent='';
        const sbImg = document.getElementById('sbAvatarImg'); sbImg.src=src; sbImg.style.display='block'; document.getElementById('sbAvatar').childNodes[0].textContent='';
        const tbImg = document.getElementById('topbarAvatarImg'); tbImg.src=src; tbImg.style.display='block'; document.getElementById('topbarAvatarLetter').style.display='none';
    };
...
(function() {
    ...
    function updateButtons(activeTheme) {
        document.querySelectorAll('.theme-opt').forEach(btn => btn.classList.toggle('on', btn.getAttribute('data-theme')===activeTheme));
    }
    window.setTheme = function(theme) { localStorage.setItem(THEME_KEY, theme); applyTheme(theme); updateButtons(theme); };
    const stored = localStorage.getItem(THEME_KEY) || 'system';
    applyTheme(stored); updateButtons(stored);
})();
...
async function initTrend() {
    ...
    try {
        const res = await fetch(`${API_URL}/admin/hourly-trend`, { headers:{'Authorization':`Bearer ${token}`,'Accept':'application/json'} });
        const data = await res.json();
        data.forEach(item => { const idx=allHours.indexOf(parseInt(item.hour)); if(idx!==-1) values[idx]=item.total; });
    } catch(e) {}
    tc = new Chart(ctx, { type:'bar', data:{ labels, datasets:[{ label:'Votes', data:values, backgroundColor:'rgba(10,48,24,.75)', hoverBackgroundColor:'rgba(5,28,14,.9)', borderRadius:5, borderSkipped:false }] }, options:{ responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{ y:{beginAtZero:true,grid:{color:gridColor},ticks:{font:chartFont,color:tickColor}}, x:{grid:{display:false},ticks:{font:chartFont,color:tickColor}} } } });
}
...
function initResults(data) {
    ...
    rc = new Chart(ctx, { type:'bar', indexAxis:'y', data:{ labels, datasets:[{ label:'Votes', data:values, backgroundColor:'rgba(10,48,24,.8)', borderRadius:5, borderSkipped:false }] }, options:{ responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{ x:{beginAtZero:true,grid:{color:gridColor},ticks:{font:chartFont,color:tickColor}}, y:{grid:{display:false},ticks:{font:chartFont,color:tickColor}} } } });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (facultySelect) {
        facultySelect.addEventListener('change', () => {
            ...
            if (activeTab) filterV(activeTab.textContent.includes('Not')?'not':activeTab.textContent.includes('Voted')&&!activeTab.textContent.includes('Not')?'voted':'all', activeTab);
        });
    }
    ...
    if (searchInput) {
        searchInput.addEventListener('input', () => {
            ...
            const activeTab = document.querySelector('.vt.on');
            let filtered = allVoters;
            if (activeTab && activeTab.textContent.includes('Not')) filtered = allVoters.filter(v => !v.votes||v.votes.length===0);
            else if (activeTab && activeTab.textContent.includes('Voted') && !activeTab.textContent.includes('Not')) filtered = allVoters.filter(v => v.votes&&v.votes.length>0);
            if (q) filtered = filtered.filter(v => v.reg_number.toLowerCase().includes(q) || (v.email&&v.email.toLowerCase().includes(q)));
            renderVoters(filtered);
        });
    }
    ...
    // Aud pills data attributes
    const audPills = document.querySelectorAll('.aud-pill');
    const audValues = ['all','voters','candidates'];
    audPills.forEach((pill, i) => { if (!pill.dataset.aud) pill.dataset.aud = audValues[i] || 'all'; });
});
